...
def earth_full_combo_finder(natal_earth, external_earth, tri):
    ...
    if tri[0] in natal_d and tri[1] in natal_d and tri[2] in external_d:
        breakdown.append(f"Natal {tri[0]} and {tri[1]} Combines with External {tri[2]}: x" + str(3 * natal_d[tri[0]] * natal_d[tri[1]] * external_d[tri[2]]))
        count+=3 * (natal_d[tri[0]] * natal_d[tri[1]] * external_d[tri[2]])
    if tri[1] in natal_d and tri[2] in natal_d and tri[0] in external_d:
        breakdown.append(f"Natal {tri[1]} and {tri[2]} Combines with External {tri[0]}: x" + str(3 * natal_d[tri[1]] * natal_d[tri[2]] * external_d[tri[0]]))
        count+=3 * (natal_d[tri[1]] * natal_d[tri[2]] * external_d[tri[0]])
    if tri[2] in natal_d and tri[0] in natal_d and tri[1] in external_d:
        breakdown.append(f"Natal {tri[2]} and {tri[0]} Combines with External {tri[1]}: x" + str(3 * natal_d[tri[2]] * natal_d[tri[0]] * external_d[tri[1]]))
        count+=3 * (natal_d[tri[2]] * natal_d[tri[0]] * external_d[tri[1]])    
    
    if tri[0] in external_d and tri[1] in external_d and tri[2] in natal_d:
        breakdown.append(f"Natal {tri[2]} Combines with External {tri[0]} and {tri[1]}: x" + str(3 * natal_d[tri[2]] * external_d[tri[0]] * external_d[tri[1]]))
    ...
    if tri[1] in external_d and tri[2] in external_d and tri[0] in natal_d:
        breakdown.append(f"Natal {tri[0]} Combines with External {tri[1]} and {tri[2]}: x" + str(3 * natal_d[tri[0]] * external_d[tri[1]] * external_d[tri[2]]))
    ...
    if tri[2] in external_d and tri[0] in external_d and tri[1] in natal_d:
        breakdown.append(f"Natal {tri[1]} Combines with External {tri[2]} and {tri[0]}: x" + str(3 * natal_d[tri[1]] * external_d[tri[2]] * external_d[tri[0]]))
...
def earth_two_third_combo_finder(natal_earth, external_earth, tri):
    ...
    if tri[0] in natal_d and tri[1] in external_d:
        breakdown.append(f"Natal {tri[0]} Combines with External {tri[1]}: x" + str(natal_d[tri[0]] * external_d[tri[1]]))
        count+=natal_d[tri[0]] * external_d[tri[1]]
    if tri[1] in natal_d and tri[2] in external_d:
        breakdown.append(f"Natal {tri[1]} Combines with External {tri[2]}: x" + str(natal_d[tri[1]] * external_d[tri[2]]))
        count+=natal_d[tri[1]] * external_d[tri[2]]
    if tri[2] in natal_d and tri[0] in external_d:
        breakdown.append(f"Natal {tri[2]} Combines with External {tri[0]}: x" + str(natal_d[tri[2]] * external_d[tri[0]]))
        count+=natal_d[tri[2]] * external_d[tri[0]]
    if tri[0] in natal_d and tri[2] in external_d:
        breakdown.append(f"Natal {tri[0]} Combines with External {tri[2]}: x" + str(natal_d[tri[0]] * external_d[tri[2]]))
        count+=natal_d[tri[0]] * external_d[tri[2]]
    if tri[1] in natal_d and tri[0] in external_d:
        breakdown.append(f"Natal {tri[1]} Combines with External {tri[0]}: x" + str(natal_d[tri[1]] * external_d[tri[0]]))
        count+=natal_d[tri[1]] * external_d[tri[0]]
    if tri[2] in natal_d and tri[1] in external_d:
        breakdown.append(f"Natal {tri[2]} Combines with External {tri[1]}: x" + str(natal_d[tri[2]] * external_d[tri[1]]))
