...
def count_earth_uncivil_punish(natal_earth, external_earth):
    ...
    for pair in [["Zi", "Mao"]]:
        if pair[0] in natal_d and pair[1] in external_d:
            breakdown.append(
                f"Natal {pair[0]} Punishes with External {pair[1]}: x{str(natal_d[pair[0]] * external_d[pair[1]])}")
            count += natal_d[pair[0]] * external_d[pair[1]]
        if pair[1] in natal_d and pair[0] in external_d:
            breakdown.append(
                f"Natal {pair[1]} Punishes with External {pair[0]}: x{str(natal_d[pair[1]] * external_d[pair[0]])}")
...
def earth_full_bully_punish_finder(natal_earth, external_earth, tri):
    ...
    if tri[0] in natal_d and tri[1] in natal_d and tri[2] in external_d:
        breakdown.append(f"Natal {tri[0]} and {tri[1]} Bully Punishes with External {tri[2]}: x" + str(
    ...
    if tri[1] in natal_d and tri[2] in natal_d and tri[0] in external_d:
        breakdown.append(f"Natal {tri[1]} and {tri[2]} Bully Punishes with External {tri[0]}: x" + str(
    ...
    if tri[2] in natal_d and tri[0] in natal_d and tri[1] in external_d:
        breakdown.append(f"Natal {tri[2]} and {tri[0]} Bully Punishes with External {tri[1]}: x" + str(
    ...
    if tri[0] in external_d and tri[1] in external_d and tri[2] in natal_d:
        breakdown.append(f"Natal {tri[2]} Bully Punishes with External {tri[0]} and {tri[1]}: x" + str(
    ...
    if tri[1] in external_d and tri[2] in external_d and tri[0] in natal_d:
        breakdown.append(f"Natal {tri[0]} Bully Punishes with External {tri[1]} and {tri[2]}: x" + str(
    ...
    if tri[2] in external_d and tri[0] in external_d and tri[1] in natal_d:
        breakdown.append(f"Natal {tri[1]} Bully Punishes with External {tri[2]} and {tri[0]}: x" + str(
...
def earth_two_third_bully_punish_finder(natal_earth, external_earth, tri):
    ...
    if tri[0] in natal_d and tri[1] in external_d:
        breakdown.append(
            f"Natal {tri[0]} Bully Punishes with External {tri[1]}: x" + str(natal_d[tri[0]] * external_d[tri[1]]))
        count += natal_d[tri[0]] * external_d[tri[1]]
    if tri[1] in natal_d and tri[2] in external_d:
        breakdown.append(
            f"Natal {tri[1]} Bully Punishes with External {tri[2]}: x" + str(natal_d[tri[1]] * external_d[tri[2]]))
        count += natal_d[tri[1]] * external_d[tri[2]]
    if tri[2] in natal_d and tri[0] in external_d:
        breakdown.append(
            f"Natal {tri[2]} Bully Punishes with External {tri[0]}: x" + str(natal_d[tri[2]] * external_d[tri[0]]))
        count += natal_d[tri[2]] * external_d[tri[0]]
    if tri[0] in natal_d and tri[2] in external_d:
        breakdown.append(
            f"Natal {tri[0]} Bully Punishes with External {tri[2]}: x" + str(natal_d[tri[0]] * external_d[tri[2]]))
        count += natal_d[tri[0]] * external_d[tri[2]]
    if tri[1] in natal_d and tri[0] in external_d:
        breakdown.append(
            f"Natal {tri[1]} Bully Punishes with External {tri[0]}: x" + str(natal_d[tri[1]] * external_d[tri[0]]))
        count += natal_d[tri[1]] * external_d[tri[0]]
    if tri[2] in natal_d and tri[1] in external_d:
        breakdown.append(
            f"Natal {tri[2]} Bully Punishes with External {tri[1]}: x" + str(natal_d[tri[2]] * external_d[tri[1]]))
...
def earth_full_ungrateful_punish_finder(natal_earth, external_earth, tri):
    ...
    breakdown = []
    if tri[0] in natal_d and tri[1] in natal_d and tri[2] in external_d:
        breakdown.append(f"Natal {tri[0]} and {tri[1]} Ungrateful Punishes with External {tri[2]}: x" + str(
            3 * natal_d[tri[0]] * natal_d[tri[1]] * external_d[tri[2]]))
        count += 3 * (natal_d[tri[0]] * natal_d[tri[1]] * external_d[tri[2]])
    if tri[1] in natal_d and tri[2] in natal_d and tri[0] in external_d:
        breakdown.append(f"Natal {tri[1]} and {tri[2]} Ungrateful Punishes with External {tri[0]}: x" + str(
            3 * natal_d[tri[1]] * natal_d[tri[2]] * external_d[tri[0]]))
        count += 3 * (natal_d[tri[1]] * natal_d[tri[2]] * external_d[tri[0]])
    if tri[2] in natal_d and tri[0] in natal_d and tri[1] in external_d:
        breakdown.append(f"Natal {tri[2]} and {tri[0]} Ungrateful Punishes with External {tri[1]}: x" + str(
            3 * natal_d[tri[2]] * natal_d[tri[0]] * external_d[tri[1]]))
        count += 3 * (natal_d[tri[2]] * natal_d[tri[0]] * external_d[tri[1]])

    if tri[0] in external_d and tri[1] in external_d and tri[2] in natal_d:
        breakdown.append(f"Natal {tri[2]} Ungrateful Punishes with External {tri[0]} and {tri[1]}: x" + str(
            3 * natal_d[tri[2]] * external_d[tri[0]] * external_d[tri[1]]))
        count += 3 * (external_d[tri[0]] * external_d[tri[1]] * natal_d[tri[2]])
    if tri[1] in external_d and tri[2] in external_d and tri[0] in natal_d:
        breakdown.append(f"Natal {tri[0]} Ungrateful Punishes with External {tri[1]} and {tri[2]}: x" + str(
            3 * natal_d[tri[0]] * external_d[tri[1]] * external_d[tri[2]]))
        count += 3 * (external_d[tri[1]] * external_d[tri[2]] * natal_d[tri[0]])
    if tri[2] in external_d and tri[0] in external_d and tri[1] in natal_d:
        breakdown.append(f"Natal {tri[1]} Ungrateful Punishes with External {tri[2]} and {tri[0]}: x" + str(
            3 * natal_d[tri[1]] * external_d[tri[2]] * external_d[tri[0]]))
        count += 3 * (external_d[tri[2]] * external_d[tri[0]] * natal_d[tri[1]])
    return count, breakdown
...
def earth_two_third_ungrateful_punish_finder(natal_earth, external_earth, tri):
    ...
    if tri[0] in natal_d and tri[1] in external_d:
        breakdown.append(
            f"Natal {tri[0]} Ungrateful Punishes with External {tri[1]}: x" + str(natal_d[tri[0]] * external_d[tri[1]]))
        count += natal_d[tri[0]] * external_d[tri[1]]
    if tri[1] in natal_d and tri[2] in external_d:
        breakdown.append(
            f"Natal {tri[1]} Ungrateful Punishes with External {tri[2]}: x" + str(natal_d[tri[1]] * external_d[tri[2]]))
        count += natal_d[tri[1]] * external_d[tri[2]]
    if tri[2] in natal_d and tri[0] in external_d:
        breakdown.append(
            f"Natal {tri[2]} Ungrateful Punishes with External {tri[0]}: x" + str(natal_d[tri[2]] * external_d[tri[0]]))
        count += natal_d[tri[2]] * external_d[tri[0]]
    if tri[0] in natal_d and tri[2] in external_d:
        breakdown.append(
            f"Natal {tri[0]} Ungrateful Punishes with External {tri[2]}: x" + str(natal_d[tri[0]] * external_d[tri[2]]))
        count += natal_d[tri[0]] * external_d[tri[2]]
    if tri[1] in natal_d and tri[0] in external_d:
        breakdown.append(
            f"Natal {tri[1]} Ungrateful Punishes with External {tri[0]}: x" + str(natal_d[tri[1]] * external_d[tri[0]]))
        count += natal_d[tri[1]] * external_d[tri[0]]
    if tri[2] in natal_d and tri[1] in external_d:
        breakdown.append(
            f"Natal {tri[2]} Ungrateful Punishes with External {tri[1]}: x" + str(natal_d[tri[2]] * external_d[tri[1]]))
        count += natal_d[tri[2]] * external_d[tri[1]]
...
def count_earth_ungrateful_punish(natal_earth, external_earth):
    ...
    if yss_count == 0:
        count, breakdown_2 = earth_two_third_ungrateful_punish_finder(natal_earth, external_earth,
                                                                      ["Yin", "Si", "Shen"])
