...
def main():
    ...
    if 'start_date' not in session.keys():
        ...
        dates = ExternalPillars.query.filter(
            ...
            ExternalPillars.date <= session["end_date"]).all()
    ...
    if request.method == 'GET':
        if current_user.natal_chart_id is None:
            ...
            return render_template('timeline.jinja2', form=Dateform, user_chart=chart,
                                   ...
                                   session=session, data=data[window_size:],
                                   labels=[d.date for d in dates[window_size:]],
                                   avg_combine=avg_combine, avg_clash=avg_clash,
                                   rolling_data=rolling_data_comb[window_size:],
                                   rolling_data_clash=rolling_data_clash[window_size:])

        if "contact_chart" not in session.keys() or (
                "contact_chart" in session.keys() and session['contact_chart'] is None):
    ...
    display_chart = 'start_date' in session.keys() and 'end_date' in session.keys() and session[
        "start_date"] is not None and session[
                        "end_date"] is not None and 'contact_name' in session.keys() and 'contact_name_id' in session.keys()
    if display_chart:
        num_days = ExternalPillars.query.filter(ExternalPillars.date >= session["start_date"],
                                                ExternalPillars.date <= session["end_date"]).count()
    ...
    return render_template('timeline.jinja2', form=Dateform, user_chart=chart,
                           ...
                           avg_combine=avg_combine, avg_clash=avg_clash, rolling_data=rolling_data_comb[window_size:],
                           rolling_data_clash=rolling_data_clash[window_size:])
